                ...
                Card {
                    Column (Modifier.fillMaxWidth().padding(8.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(4.dp)) {
                        Row(
                            ...
                            Text(text = it.nameDay.name, style = MaterialTheme.typography.headlineMedium, color = MaterialTheme.colorScheme.primary)
                            Text(text = it.nameDay.date.toString(), textAlign = TextAlign.End, style = MaterialTheme.typography.headlineMedium, color = MaterialTheme.colorScheme.secondary)
                        }
                        HorizontalDivider()
                        Text(text = "Atgāginājumi:", style = MaterialTheme.typography.titleLarge)
                        it.reminders.forEach {
                            Card(modifier = Modifier.padding(8.dp)) {
                                Row(
                                    ...
                                    Text(text = it.dateToRemind.toString(), style = MaterialTheme.typography.bodyLarge)
